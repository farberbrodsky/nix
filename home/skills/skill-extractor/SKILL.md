...
- Default: saves to `/persist/nix/home/skills/[name]/SKILL.md`
    - Then, ask the user to review the skill, before it can be integrated into the active configuration!
- `--project`: saves to `.agents/skills/[name]/SKILL.md`
...
# Search by keyword
grep -r "keyword" ~/.pi/agent/skills/ .agents/skills/ 2>/dev/null
```
...
- User-level: `/persist/nix/home/skills/[name]/SKILL.md`
...
```
For global skills, since they are NOT installed automatically, report:
```
User, please review the skill and then integrate it into the running configuration.
```
